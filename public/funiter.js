...
funiter.start = (expressionParser, state) => {
    if (expressionParser)
...
const play = (autoPlay = true) => {
    ...
    if (isRevaluation && currency.nbMembers > 0) {
        meltValue = funiter.expressionParser(currency.expression, {M: currency.monetaryMass / uGained, N: currency.nbMembers, T: currency.elapsedTime})
